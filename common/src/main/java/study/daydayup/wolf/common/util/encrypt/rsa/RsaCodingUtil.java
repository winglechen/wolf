package study.daydayup.wolf.common.util.encrypt.rsa;
...
public final class RsaCodingUtil {
    ...
    public static String encryptByPublicKey(String src, PublicKey publicKey) {
        ...
        return SecurityUtil.byte2Hex(destBytes);
    ...
    public static String decryptByPriPfxFile(String src, String pfxpath, String priKeyPass) {
        if (StringUtil.isEmpty(src) || StringUtil.isEmpty(pfxpath)) {
            return null;
        }
    ...
    public static String decryptByPrivateKey(String src, PrivateKey privateKey) {
        ...
        try {
            byte[] destBytes = rsaByPrivateKey(SecurityUtil.hex2Bytes(src), privateKey, Cipher.DECRYPT_MODE);
            if (destBytes == null) {
                return null;
            }
...
//			//log.error("解密内容异常", e);
    ...
    public static String decryptByPublicKey(String src, PublicKey publicKey) {
        ...
        try {
            byte[] destBytes = rsaByPublicKey(SecurityUtil.hex2Bytes(src), publicKey, Cipher.DECRYPT_MODE);
            if (destBytes == null) {
                return null;
            }
