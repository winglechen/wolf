 ...
 **/
@RestController
public class UnionStatusController  {
    @Resource
    ...
    @GetMapping("/status/customer/info")
    public Map<String, Boolean> customerInfo() {
        agent.init(session.get("accountId", Long.class), session.get("orgId", Long.class));

        agent.set(CustomerInfoEnum.BASIC_INFO);
        agent.set(CustomerInfoEnum.BANK_CARD);
        agent.set(CustomerInfoEnum.LIVENESS);
        agent.set(CustomerInfoEnum.AADHAAR);
        agent.save();
