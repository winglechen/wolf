...
public class ConsumerManager {
    ...
    public void start(String name, MQConsumerConfig config) {
        ...
        SubscriptionRegistry.register(
            config.getGroup(),
            config.getTopics().iterator().next(),
            CollectionUtil.notEmpty(config.getTags()) ? config.getTags().iterator().next() : null
        );
        ...
        for (int i = 1; i <= config.getGroupConfig().getConsumerNum(); i++) {
            ...
            log.info("[MQ] consumer has been started. consumerId={}, vendor={} group={}, topics={}, tags={} minThreadNum={}, maxThreadNum={}",
